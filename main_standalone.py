#!/usr/bin/env python3
"""
Автономная версия YOVPN Telegram Bot (без Marzban API)
...
def show_main_menu(message):
    ...
    # В автономном режиме всегда показываем как нового пользователя
...
# Все остальные функции остаются такими же, как в main_improved.py
# (копируем их для автономной работы)
...
def show_my_subscriptions(message):
    ...
    # В автономном режиме показываем заглушку
    text = f"""
{EMOJI['subscription']} <b>Мои подписки</b>

{EMOJI['warning']} <b>Автономный режим</b>
Marzban API недоступен. Подписки не отображаются.

{EMOJI['info']} <b>Для полной функциональности:</b>
1. Настройте реальные токены в .env
2. Запустите main_improved.py
"""
...
# Добавляем остальные функции как заглушки
@handle_error
...
if __name__ == "__main__":
    logger.info("Запуск автономного бота (без Marzban API)...")
